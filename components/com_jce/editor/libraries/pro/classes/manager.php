...
class WFMediaManager extends WFMediaManagerBase
{
    ...
    protected function getThumbName($file)
    {
        ...
        $value = $this->getParam('editor.thumbnail_prefix', 'thumb_$');

        // legacy value from Image Manager Extended parameters
        $value = $this->getParam('imgmanager_ext.thumbnail_prefix', $value);

        if (strpos($value, '$') !== false) {
            return str_replace('$', basename($file, '.' . $ext), $value) . '.' . $ext;
        }
    ...
    protected function getThumbDir($file, $create)
    {
        ...
        // get global parameter
        $folder = $this->getParam('editor.thumbnail_folder', '', 'thumbnails', 'string', true);

        // legacy value from Image Manager Extended parameters
        $folder = $this->getParam('imgmanager_ext.thumbnail_folder', $folder, 'thumbnails', 'string', true);
