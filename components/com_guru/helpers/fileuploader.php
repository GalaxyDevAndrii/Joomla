...
class qqFileUploader {
    ...
    function handleUpload($uploadDirectory, $replaceOldFile = FALSE){
        ...
        if ($this->file->save($uploadDirectory . $filename . '.' . $ext)){
        	$file_size = filesize($uploadDirectory . $filename . '.' . $ext);

        	if(!isset($_SERVER["CONTENT_LENGTH"])){
        		$_SERVER["CONTENT_LENGTH"] = $file_size;
        	}
			...
			$accepted_files = array("doc", "docx", "txt", "pdf", "csv", "htm", "html", "xhtml", "sxi", "xml", "sxw", "rtf", "odt", "css", "odp", "pps", "ppt", "ax", "amv", "asf", "avi", "mp4", "fla", "swf", "mov", "mpg", "mpeg", "rm", "aac", "aob", "ada", "au", "mid", "midi", "mp3", "ogg", "wav", "jpg", "jpeg", "png", "gif", "zip", "m4a", "xls", "xlsx");
...
switch ($mediaType){
	...
	case "file":
		...
		break;
	case "project":
		if(!is_dir(JPATH_BASE.DIRECTORY_SEPARATOR."media".DIRECTORY_SEPARATOR."projects")){
			jimport('joomla.filesystem.folder');
			jimport('joomla.filesystem.file');

			JFolder::create(JPATH_BASE.DIRECTORY_SEPARATOR."media".DIRECTORY_SEPARATOR."projects");
			JFile::write(JPATH_BASE.DIRECTORY_SEPARATOR."media".DIRECTORY_SEPARATOR."projects".DIRECTORY_SEPARATOR."index.html", "");
		}

		$user = JFactory::getUser();
		$user_id = $user->id;

		if(!is_dir(JPATH_BASE.DIRECTORY_SEPARATOR."media".DIRECTORY_SEPARATOR."projects".DIRECTORY_SEPARATOR.intval($user_id))){
			jimport('joomla.filesystem.folder');
			jimport('joomla.filesystem.file');

			JFolder::create(JPATH_BASE.DIRECTORY_SEPARATOR."media".DIRECTORY_SEPARATOR."projects".DIRECTORY_SEPARATOR.intval($user_id));
			JFile::write(JPATH_BASE.DIRECTORY_SEPARATOR."media".DIRECTORY_SEPARATOR."projects".DIRECTORY_SEPARATOR.intval($user_id).DIRECTORY_SEPARATOR."index.html", "");
		}

		$uploadPath = JPATH_BASE.DIRECTORY_SEPARATOR."media".DIRECTORY_SEPARATOR."projects".DIRECTORY_SEPARATOR.intval($user_id).DIRECTORY_SEPARATOR;
		break;
	...
	case "scorm":
		...
		if(!is_dir($uploadPath)){
			...
			JFile::write($uploadPath."index.html", "");
		}
...
if(isset($result['success'])&&($result['success'] == true)) {
	...
	//start others types; don't need to do thumbs
		...
		$file_request = JFactory::getApplication()->input->get('qqfile', "", "raw");

		if($mediaType == "project"){
			$result['locate'] = $file_request;

			$user = JFactory::getUser();
			$user_id = $user->id;
			$course_id = JFactory::getApplication()->input->get('course_id', "0", "raw");
			$project_id = JFactory::getApplication()->input->get('project_id', "0", "raw");
			$lesson_id = JFactory::getApplication()->input->get('lesson_id', "0", "raw");

			$db = JFactory::getDbo();
			$sql = "select count(*) from #__guru_project_results where `project_id`=".intval($project_id)." and `student_id`=".intval($user_id)." and `course_id`=".intval($course_id)." and `lesson_id`=".intval($lesson_id);
			$db->setQuery($sql);
			$db->execute();
			$count = $db->loadColumn();
			$count = @$count["0"];
			...
			$now_date = $jnow->toSQL(true);

			if(intval($count) == 0){
				$sql = "insert into #__guru_project_results (`project_id`, `student_id`, `course_id`, `lesson_id`, `file`, `score`, `desc`, `created_date`) values ('".intval($project_id)."', '".intval($user_id)."', '".intval($course_id)."', '".intval($lesson_id)."', '".trim($file_request)."', '0', '', '".$now_date."')";
				$db->setQuery($sql);
				$db->execute();
			}
			else{
				$sql = "update #__guru_project_results set `file`='".trim($file_request)."' where `student_id`=".intval($user_id)." and `course_id`=".intval($course_id)." and `lesson_id`=".intval($lesson_id)." and `project_id`=".intval($project_id);
				$db->setQuery($sql);
				$db->execute();
			}
		}
