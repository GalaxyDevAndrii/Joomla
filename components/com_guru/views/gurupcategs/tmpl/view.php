...
defined( '_JEXEC' ) or die( 'Restricted access' );
    ...
    //$document->setTitle(JText::_("GURU_PROGRAM_CATEGORIES")." - ".$categ->name);
    $document->setTitle($categ->name);
        ...
        if(isset($childrens) && is_array($childrens) && count($childrens) > 0){
            ...
            foreach($childrens as $key=>$value){
                ...
                if($result["0"] != "0"){
                    ...
                    echo $next_nr." ".'<a href="'.JUri::root().'index.php?option=com_guru&view=gurupcategs&task=view&cid='.$value["id"]."-".JFilterOutput::stringURLSafe(trim($value["name"])).'&Itemid='.intval($item_id_categ).'">'.trim($value["name"])." (".$result["0"].")".'</a>';
...
function generateCategsCellsC($config_categs, $style_categs, $course, $config){
    ...
    if(intval($itemid_req) != 0){
        $db = JFactory::getDbo();
        $sql = "select `params` from #__menu where `id`=".intval($itemid_req);
        $db->setQuery($sql);
        $db->execute();
        ...
        if(isset($menu_params) && trim($menu_params) != ""){
            ...
            if(isset($menu_params["display"]) && ($menu_params["display"] == "tree" || $menu_params["display"] == "listing")){
                if($menu_params["display"] == "tree"){
                    $layout = "0";
                }
                elseif($menu_params["display"] == "listing"){
                    $layout = "1";
                }
            }
        }
    ...
    if ($show === true) {
        ...
        if ($layout == "1") {
            ...
            if(trim($course->image) != ""){
                $image = '<img alt="Category Image" class="uk-border-rounded wk-border-rounded '.$style_categs->ctgs_image.'" src="'.JURI::root().$course->image.'" />';
            ...
            $return .= '<div class="uk-panel uk-panel-box wk-panel wk-panel-box clearfix">';
            ...
            if ($img_align == 0) { // Centered image
                ...
                if(trim($image) != ""){
                    ...
                    $return .= '<div class="uk-panel-teaser wk-panel-teaser gru-panel-teaser" style="background-image: url(\''.JURI::root().$course->image.'\');">
                        <a href="'.$image_link.'"></a>
                    </div>';
                }

                $return .= '<div class="uk-panel-badge uk-badge uk-badge-notification uk-badge-warning wk-panel-badge wk-badge wk-badge-notification wk-badge-warning">'.$edit_sum.'</div>';

                $return .= '<h3 class="uk-panel-title wk-panel-title '.$style_categs->ctgs_categ_name.'">
                ...
                if($read_more == "0" && $edit_read_more == "0"){
                    $rmore = '<a style="float:'.$read_align.'" class="uk-button wk-button" href="'.JRoute::_('index.php?option=com_guru&view=gurupcategs&task=view&cid='.$course->id."-".$alias."&Itemid=".intval($item_id_categ)).'">'.JText::_("GURU_READ_MORE").'</a>';
                }
                ...
                $return .= '<div class="uk-grid uk-grid-small wk-grid wk-grid-small">';

                $return .= '<div class="uk-badge uk-panel-badge uk-badge-warning wk-badge wk-panel-badge wk-badge-warning">'.$edit_sum.'</div>';

                $return .= '<div class="uk-width-1-1 wk-width-1-1">
                                <div class="uk-panel-header wk-panel-header">
                                    <h3 class="uk-panel-title wk-panel-title '.$style_categs->ctgs_categ_name.'">
                                        <a href="'.JRoute::_('index.php?option=com_guru&view=gurupcategs&task=view&cid='.$course->id."-".$alias."&Itemid=".intval($item_id_categ)).'">'.$course->name.'</a>
                                    </h3>
                                </div>
                            </div>';
                ...
                if(trim($image) != ""){
                    $return .= '<div class="uk-width-1-1 uk-width-medium-1-3 wk-width-1-1 wk-width-medium-1-3 '.$style_categs->ctgs_image.'">'.$image.'</div>';
                }

                $return .= '<div class="uk-width-1-1 uk-width-medium-2-3 wk-width-1-1 wk-width-medium-2-3">';
...
function generateCoursesCellsB($config_courses, $style_courses, $course, $config) {
    ...
    if($layout == "1"){//mini profile
        ...
        if (trim($course->image_avatar) != ""){
            ...
            $image_rounded = '<img class="uk-border-rounded wk-border-rounded" src="'.JURI::root().$course->image_avatar.'" />';
        ...
        $return .= '<div class="uk-panel uk-panel-box uk-panel-box-secondary wk-panel wk-panel-box wk-panel-box-secondary">';
        ...
        if ($img_align == "0") {
            ...
            if (trim($image) != "") {
                ...
                $return .= '<div class="uk-panel-teaser wk-panel-teaser gru-panel-teaser uk-cover-background wk-cover-background" style="background-image: url(\''.$image_url.'\');">
                                <a href="'.$image_link.'"></a>
                                <figure class="uk-overlay wk-overlay">
                                    <figcaption class="uk-overlay-panel uk-overlay-bottom uk-overlay-background wk-overlay-panel wk-overlay-bottom wk-overlay-background">
                                        <ul class="uk-list uk-padding-remove wk-list wk-padding-remove">
                                            <li class="uk-display-inline-block wk-display-inline-block">
                                                <i class="uk-icon-user wk-icon-user"></i> '.$authors_urls.'
                                            </li>';
                if(trim($count_courses_text) != ""){
                    $return .= '
                                            <li class="uk-display-inline-block wk-display-inline-block" title="'.$count_courses_text_tip.'" data-uk-tooltip>
                                                <i class="uk-icon-graduation-cap wk-icon-graduation-cap"></i> '.$count_courses_text.'
                                            </li>';
                }
                ...
                if($show_price == 0){
                    $return .= '<div class="uk-panel-badge uk-badge uk-badge-success uk-badge-notification wk-panel-badge wk-badge wk-badge-success wk-badge-notification">'.$price.'</div>';
                }
                ...
                $return .= '<div class="'.$style_courses->courses_name.'">
                                <h3 class="uk-panel-title wk-panel-title"><a href="'.JRoute::_('index.php?option=com_guru&view=guruPrograms&task=view&cid='.$course->id."-".$alias."&Itemid=".intval($item_id)).'">'.$course->name.'</a></h3>
                            </div>';

                if ($read_more == "0" && $edit_read_more == "0") {
                    // : Show `Read more` button depending on admin settings
                    $rt =  '<div class="readon"><a class="uk-button wk-button" style="float:'.$read_align.'" href="'.JRoute::_('index.php?option=com_guru&view=guruPrograms&task=view&cid='.$course->id."-".$alias."&Itemid=".intval($item_id)).'">'.JText::_("GURU_READ_MORE").'</a></div>';
                }
        ...
        elseif ($img_align == "1") {
            ...
            $return .=     '<div class="uk-width-1-1 uk-margin-bottom wk-width-1-1 wk-margin-bottom '.$style_courses->courses_name.'">
                                <h3 class="uk-panel-title wk-panel-title"><a href="'.JRoute::_('index.php?option=com_guru&view=guruPrograms&task=view&cid='.$course->id."-".$alias."&Itemid=".intval($item_id)).'">'.$course->name.'</a></h3>
                                <ul class="uk-list wk-list gru-course-box-details">
                                    <li class="uk-display-inline-block wk-display-inline-block"><i class="uk-icon-user wk-icon-user"></i> '.$authors_urls.' </li>';

            if(trim($count_courses_text) != ""){
                $return .=     '
                                    <li class="uk-display-inline-block wk-display-inline-block" title="'.$count_courses_text_tip.'" data-uk-tooltip><i class="uk-icon-graduation-cap wk-icon-graduation-cap"></i> '.$count_courses_text.'</li>';
            }
            ...
            if (trim($image) != "") {
                $return .= '<div class="uk-width-1-1 uk-width-medium-1-3 wk-width-1-1 wk-width-medium-1-3">'.$image_left.'</div>';
            }

            $return .=     '<div class="uk-width-medium-2-3 wk-width-medium-2-3">';
        ...
        elseif ($img_align == "2") {
            ...
            if (trim($image) != "") {
                $return .= '<div class="uk-width-1-1 uk-width-medium-1-3 wk-width-1-1 wk-width-medium-1-3">'.$image_right.'</div>';
            }
    ...
    if($categ->published == 1){
        ...
        if(trim($categ->image) != ""){
            if ($config_category->ctg_image_alignment == "1") {
                $categ_image = '<img border="0" alt="" class="uk-border-rounded wk-border-rounded" src="'.JURI::root().$categ->image.'" />';
            }
        ...
        $desc = $this->cutDescription($categ->description, $config_category->ctg_description_length, $config_category->ctg_description_type, $config_category->ctg_description_mode);
            ...
            <?php
                ...
                '<div id="g_cath_detail_top" class="cat_level_wrap cat_level_'.$var_inc.' g_sect clearfix">';
                    $category_layout .= 
                       '<div class="uk-panel uk-panel-box wk-panel wk-panel-box">';

                            if ($config_category->ctg_image_alignment == "0") { 
                                if($categ_image != ""){
                                    $category_layout .= '<div class="uk-panel-teaser uk-cover-background wk-panel-teaser wk-cover-background gru-category-teaser" style="background-image: url('.$categ_image_url.');">
                                        <figure class="uk-overlay wk-overlay">
                                            <figcaption class="uk-overlay-panel uk-overlay-background uk-overlay-bottom wk-overlay-panel wk-overlay-background wk-overlay-bottom"><h3>' . $categ->name . '</h3></figcaption>
                                        </figure>
                            ...
                            elseif ($config_category->ctg_image_alignment == "1") {
                                $category_layout .= '<div class="uk-grid wk-grid">';
                                if($categ_image != ""){
                                    $category_layout .= '<div class="uk-width-1-1 uk-width-medium-1-3 wk-width-1-1 wk-width-medium-1-3">'.$categ_image.'</div>'; //main category image                                                           
                                }

                                $category_layout .=   '<div class="uk-width-1-1 uk-width-medium-2-3 wk-width-1-1 wk-width-medium-2-3">';
                    ...
                    if(isset($subcateg) && count($subcateg) > 0){
                        ...
                        if($layout == "0"){
                            ...
                            while(isset($categs_array[$i])){
                                $row = "";
                                $row .= '<div class="cont_detail_guru uk-grid wk-grid" >';
                                ...
                                while($j < $cols){
                                    ...
                                    elseif(isset($categs_array[$i]) && trim($categs_array[$i]) != ""){  
                                        $row .= '<div class="course_cell_guru g_cell uk-width-large-1-'.$cols.' wk-width-large-1-'.$cols.' uk-width-medium-1-'.$cols.' wk-width-medium-1-'.$cols.'">
                ...
                if($layout == "0"){
                    if(count($programs) > 0){
                        ...
                        $form_url = JUri::root(true) . "/index.php?option=com_guru&view=gurupcategs&task=view&cid=".$cid."&Itemid=".$item_id_categ;
                    
                        $category_layout .= '<form name="adminForm" id="adminForm" action="'.$form_url.'" method="post">';

                        $category_layout .= "<div class='uk-panel uk-panel-box uk-panel-box-secondary uk-margin-top wk-panel wk-panel-box wk-panel-box-secondary wk-margin-top'><ul class='guru-list'>";
                        ...
                        $category_layout .= '<div class="uk-grid wk-grid"><div class="uk-width-large-1-1 wk-width-large-1-1">';
                    ...
                    $cid = JFactory::getApplication()->input->get("cid", "0");
                    $Itemid = JFactory::getApplication()->input->get("Itemid", "0", "raw");
                    $guru_search = JFactory::getApplication()->input->getString("guru_search", "");
                    ...
                    if(count($courses_array) > 0){
                        $category_layout .= '<div class="uk-grid wk-grid">
                                                <div class="uk-width-1-1 wk-width-1-1">
                                                    <h4 class="gru-page-subtitle uk-margin-top wk-margin-top">'.JText::_("GURU_COURSES_IN_CATEGORY").':</h4>
                                                </div>
                                            </div>';
                    }
                    
                    $limit_pag = $this->pagination->getLimitBox();
                    $limit_pag = preg_replace('/class="(.*)"/msU', 'class="uk-margin-small-top uk-form-width-mini wk-margin-small-top wk-form-width-mini"', $limit_pag);
                    
                    $category_layout .= '<div class="gru-page-filters uk-margin wk-margin">';
                    $category_layout .=     '<div class="gru-filter-item"><input type="text" placeholder="'.JText::_("GURU_SEARCH").'" name="guru_search" value="'.trim($guru_search).'" class="form-control uk-margin-remove uk-form-width-small wk-margin-remove wk-form-width-small" /> <input type="submit" name="search_button" value="'.JText::_("GURU_SEARCH").'" class="uk-button uk-button-primary wk-button wk-button-primary" style="margin:0px;"></div>';
                    ...
                    $category_layout .= '<div class="uk-grid wk-grid" data-uk-grid-margin data-uk-grid-match="{target:\'.uk-panel\'}">';
                    while(isset($courses_array[$i])){
                        $row = "";
                        for($j=0; $j<$cols; $j++){
                            if(count($courses_array) == 1){             
                                if(isset($courses_array[$i])){
                                    $row .= '<div class="uk-width-large-1-1 wk-width-large-1-1"><div>'.$courses_array[$i++]."</div></div>";
                                }
                            }
                            else{
                                if(isset($courses_array[$i])){
                                        $row .= '<div class="uk-width-large-1-'.$cols.' wk-width-large-1-'.$cols.' uk-width-medium-1-2 wk-width-medium-1-2"><div>'.$courses_array[$i++]."</div></div>";
                                }
                            }
                        }
                        $category_layout .= $row;
