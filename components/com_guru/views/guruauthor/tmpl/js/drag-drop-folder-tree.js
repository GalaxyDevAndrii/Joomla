...
function getDomain(path) {
	var s = path.substr(0, 8), d;
	try {
		if (s.search('http://') < 0 && s.search('https://') < 0 && s.search('ftp://') < 0) { s = "http://" + path; } else { s = path; }
		d = s.match(/:\/\/(www[0-9]?\.)?(.[^/:]+)/)[2];
	}
	catch (e) {
		d = "";
...
function JSDragDropTree() {
	...
	this.imageFolder = site_url + 'components/com_guru/images/';
	...
	// this.floatingContainer.style.marginTop = '-140px';
	this.floatingContainer.id = 'floatingContainer';

	var sel_zuzu = document.getElementById('zuzuzuzu');

	document.body.appendChild(this.floatingContainer);
	// sel_zuzu.appendChild(this.floatingContainer);
	...
	this.filePathRenameItem = 'components/com_guru/views/guruauthor/tmpl/folderTree_updateItem.php';
	this.filePathDeleteItem = 'components/com_guru/views/guruauthor/tmpl/folderTree_updateItem.php';
	...
	*/
	...
	dropDragableNodes: function () {
		...
		*/
		...
		if (JSTreeObj.dragNode_destination) {
			...
			if (JSTreeObj.insertAsSub) {
				...
				if (tmpVar == 'true') // we're moving a screen
				{
					...
					if (JSTreeObj.dragNode_destination.nextSibling == null && tmpVar1 == 'false') { // but dropping on a group
					...
					else if (JSTreeObj.dragNode_destination.parentNode.parentNode.nextSibling == null && tmpVar1 == 'true') { // but dropping on a screen
						//alert('appending a group to a screen from the last group');
						var tree_root = document.getElementById('tree_ul_0');
						tree_root.appendChild(JSTreeObj.dragNode_source);
					}
					// doing a drop between two groups
					else if (JSTreeObj.dragNode_destination.nextSibling && tmpVar1 == 'false') {// dropping on a specific group
						//alert('dropping a group on an inside group');
						var nextSib = JSTreeObj.dragNode_destination.nextSibling;
						nextSib.parentNode.insertBefore(JSTreeObj.dragNode_source, nextSib);
					}
					else if (JSTreeObj.dragNode_destination.nextSibling && tmpVar1 == 'true') { // dropping on a screen from that group
		...
		var elements = document.getElementsByClassName('g_inline_child');
		...
		if (tmpVar == 'false') {
			var url = site_url + 'index.php?option=com_guru&controller=guruAuthor&tmpl=component&format=raw&task=saveOrderG&' + for_link;
			var save_type = "g";
		}
		else {
			var url = site_url + 'index.php?option=com_guru&controller=guruAuthor&tmpl=component&format=raw&task=saveOrderS&' + for_link;
			var save_type = "s";
		}
		...
		if (save_type == "g") {
			...
			ajaxObjects[ajaxIndex].onCompletion = function () {
				JSTreeObj.updateLessonOrdering();
			};
	...
	updateLessonOrdering: function() {
		var pid = jQuery('.tree-selectable').attr('pid');
		var lessionIds = [];
		
		jQuery('li.tree-leaf').each(function(idx, elm) {
			var leafid = jQuery(elm).attr('leafid');
			if (leafid) {
				lessionIds.push(leafid);
			}
		});

		var modules = [];
		jQuery('li.tree-branch').each(function(idx, branch) {
			var $branch = jQuery(branch);
			var leafIds = [];

			$branch.find('.tree-leaf').each(function(idx, leaf) {
				leafIds.push(jQuery(leaf).attr('leafid'));
			});

			modules.push({
				id: $branch.attr('leafid'),
				leafs: leafIds,
			});
		});

		var url = Joomla.getOptions('system.paths').base + '/index.php?option=com_guru&controller=guruAuthor&tmpl=component&format=raw&task=saveOrderS';
		...
		jQuery.ajax({
			...
			data: {
				pid: pid,
				ordering: JSON.stringify(lessionIds),
				modules: JSON.stringify(modules)
			}
		})
		.error(function() {
			alert('lesson ordering error');
		});
	}
	,
	createDropIndicator: function () {
		this.dropTargetIndicator = document.createElement('DIV');
		this.dropTargetIndicator.className  = 'dragDropIndicator';
		this.dropTargetIndicator.style.position = 'absolute';
		this.dropTargetIndicator.style.display = 'none';
		// this.dropTargetIndicator.style.marginTop = '-140px';
		...
		// drop_indicator.appendChild(this.dropTargetIndicator);
	}
	,
	...
	initTree: function (how_many, node_tree) {
		...
		for (var no = 0; no < menuItems.length; no++) {
			...
			if (tmpLeaf == 'false' && node_tree == 0) {
				//check_box = document.getElementById("check_all_lessons"+poz);
				check_box = document.querySelector('[id^="check_all_lessons_' + poz + '"]');
				poz++;

				menuItems[no].insertBefore(img, check_box);
			}
	...
	_____addElement: function (day_title, day_ordering, newdayid) {
		...
		new_element.innerHTML = '<img src="/administrator/components/com_guru/views/guruauthor/tmpl/images/dhtmlgoodies_minus.gif"/><a  class="modal" rel="{handler: \'iframe\', size: {x: 850, y: 650}}" id="nodeATag1000' + day_ordering + '" href="index.php?option=com_guru&controller=guruAuthor&tmpl=component&task=edit&node=1000' + day_ordering + '&cid[]=' + newdayid + '">Group ' + day_ordering + ' - ' + day_title + '</a><a class="modal" href="index.php?option=com_guru&controller=guruAuthor&task=addtask&no_html=1&cid[]=' + newdayid + '&node=' + day_ordering + '" rel="{handler: \'iframe\', size: {x: 700, y: 450}}"><font color="#0b55c4"> (add screen)</font></a><ul id="tree_ul_' + day_ordering + '" style="display: block;"></ul>';
