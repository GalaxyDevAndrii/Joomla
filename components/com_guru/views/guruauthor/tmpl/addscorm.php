...
function addscorm (idu, name, asoc_file, description) {
	...
	jQuery.ajax({
		...
		success: function(transport) {
			...
			replace_edit_link.href = Joomla.getOptions('system.paths'). root + '/index.php?option=com_guru&controller=guruAuthor&tmpl=component&task=editScorm&cid='+ idu;
	...
	<div style="float: left; font-weight:bold"><?php echo JText::_("GURU_CLICK_TO_SCORM"); ?></div>
		<br /><br />
	<div>
<form name="adminForm2" action="<?php echo JUri::root(true) ?>/index.php?option=com_guru&controller=guruAuthor&task=addscorm&med=<?php echo $data_get['med'];?>&tmpl=component&cid=<?php echo $data_get['cid'];?><?php if(isset($data_get['scorm'])){echo "&scorm=".$data_get['scorm'];}?><?php if(isset($_REQUEST['type'])){echo "&type=".$_REQUEST['type'];}?>" method="post">
	<div class="gru-page-filters">
            <div class="gru-filter-item">
				<input type="text" class="form-control" name="search_scorm" value="<?php
					...
					if(isset($data_post['search_scorm'])&&($data_post['search_scorm']!='')){
						echo $data_post['search_scorm'];
					} 
					elseif(isset($search_scorm_tskmed)&&($search_scorm_tskmed!='')) {
						echo $search_scorm_tskmed;
					}?>" />
...
<style>
	...
	<form name="adminForm" id="adminForm" action="<?php echo JUri::root(true) ?>/index.php?option=com_guru&controller=guruAuthor&task=addscorm&med=<?php echo $data_get['med'];?>&tmpl=component&cid=<?php echo $data_get['cid'];?>" method="post">
		<table>
        	<tr>
        		<td colspan="6">
