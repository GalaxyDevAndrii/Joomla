...
function addmedia (idu, name, asoc_file, description) {
	...
	<div style="float: left; font-weight:bold"><?php echo JText::_("GURU_CLICK_TO_QUIZ"); ?></div>
		<br /><br />
	<div>
<form name="adminForm2" action="<?php echo JUri::root(true) ?>/administrator/index.php?option=com_guru&controller=guruTasks&task=addquiz&med=<?php echo $data_get['med'];?>&tmpl=component&cid[]=<?php echo $data_get['cid'][0];?><?php if(isset($data_get['quiz'])){echo "&quiz=".$data_get['quiz'];}?><?php if(isset($_REQUEST['type'])){echo "&type=".$_REQUEST['type'];}?>" method="post">
...
<?php 
		...
		?>
			<tr class="camp0"> 
				...
				<td><?php if($published==1) { echo '<img src="'.JUri::base().'components/com_guru/images/tick.png" alt="Published" />';} 
					else { echo '<img src="'.JUri::base().'components/com_guru/images/publish_x.png" alt="Unpublished" />';}
					?>
				</td>		
			</tr>
		<?php 
	...
	<form name="adminForm" id="adminForm" action="<?php echo JUri::root(true) ?>/administrator/index.php?option=com_guru&controller=guruTasks&task=addquiz&med=<?php echo $data_get['med'];?>&tmpl=component&cid[]=<?php echo $data_get['cid'][0];?>" method="post">
		<table>
        	<tr>
        		<td colspan="6">
