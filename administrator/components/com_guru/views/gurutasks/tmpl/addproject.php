...
function addproject (idu, name, asoc_file, description) {
	...
	<div style="float: left; font-weight:bold"><?php echo JText::_("GURU_CLICK_TO_PROJECT"); ?></div>
		<br /><br />
	<div>
<form name="adminForm2" action="<?php echo JUri::root(true) ?>/administrator/index.php?option=com_guru&controller=guruTasks&task=addproject&med=<?php echo $data_get['med'];?>&tmpl=component&cid[]=<?php echo $data_get['cid'][0];?><?php if(isset($data_get['project'])){echo "&project=".$data_get['project'];}?><?php if(isset($_REQUEST['type'])){echo "&type=".$_REQUEST['type'];}?>" method="post">
...
<?php 
			...
			<tr class="camp0"> 
				...
				<td>
                	<?php
						...
						$model_project = new guruAdminModelguruProjects();
                    	$course = $model_project->projectCourse($id);
						echo $course;
					?>
                </td>	
			</tr>
	...
	<form name="adminForm" id="adminForm" action="<?php echo JUri::root(true) ?>/administrator/index.php?option=com_guru&controller=guruTasks&task=addproject&med=<?php echo $data_get['med'];?>&tmpl=component&cid[]=<?php echo $data_get['cid'][0];?>" method="post">
		<table>
        	<tr>
        		<td colspan="6">
