...
if ($course_config->course_price == "0") {
    ...
    if(isset($chb_free_courses) && $chb_free_courses == 1) {
        ...
        $show_course_price .= '<li><span class="fas icon fa-dollar-sign text-primary"></span> <span class="uk-text-bold">'.JText::_("GURU_BUY_PRICE").':</span> '.$text.'</li>';
    }
	else{
        if (isset($prices)) {
            if ($res["currencypos"] == '0') {
				...
				$show_course_price .= '<li><span class="fas icon fa-dollar-sign text-primary"></span> <span class="uk-text-bold">'.JText::_("GURU_BUY_PRICE").':</span> '.$prices."</li>";
            }
            else{
				if(strpos($prices, "-") !== false){
...
<div class="row view-course-detail">
    <div class="col-xs-12 col-sm-12 col-lg-4 order-lg-2">
        ...
        <div class="course-sidebar">
            <div class="uk-cover-background gru-cover" <?php echo $style; ?> ></div>
                        ...
                        <div class="uk-course-info">
                            <ul class="uk-list uk-padding-remove">
                                ...
                                if ($course_config->show_course_studentamount == "0") {
                                    echo '<li><span class="text-primary icon fas fa-graduation-cap"></span> <span class="uk-text-bold">'.JText::_("GURU_AMOUNT_STUDENTS").':</span> '.$amount_students.'</li>';
                                }
                                ...
                                if ($course_config->course_author_name_show == "0") {
                                    ...
                                    if (isset($list_author_name) && count($list_author_name) > 0) {
                                        echo '<li><span class="text-primary icon fas fa-user-tie"></span> <span class="uk-text-bold">'.JText::_("GURU_AUTHOR").':</span> '.implode(", ", $list_author_name).'</li>';
                                    }
                                }

                                if($course_config->course_released_date == "0"){
                                    echo '<li><span class="text-primary icon far fa-calendar-alt"></span> <span class="uk-text-bold">'.JText::_("GURU_RELEASED").':</span> '.$data.'</li>';
                                }
                                
                                if($course_config->course_level == "0"){
                                    echo '<li><span class="text-primary icon fas fa-signal"></span> <span class="uk-text-bold">'.JText::_("GURU_LEVEL").':</span> '.$program->level."</li>";
                                }

                                if(isset($course_config->duration) && $course_config->duration == 0){
                                    echo '<li><span class="text-primary icon far fa-clock"></span> <span class="uk-text-bold">'.JText::_("GURU_DURATION").':</span> <span id="total-duration-1"></span></li>';
                                }
                        ...
                        <div class="uk-course-action">
                            ...
                            <div class="gru-cover-purchase-btn">
                                ...
                                <?php
                                	if($need_enroll){
        						?>
                                		<a class="btn btn-block btn-primary" href="#" onclick="document.location.href='<?php echo JRoute::_("index.php?option=com_guru&view=guruprograms&task=enroll&action=enroll&cid=".$course_id); ?>'; return false;">
                                ...
                                <?php
        							...
        							elseif($need_buy){
        						?>
                                        <a class="btn btn-block btn-primary" href="<?php echo JURI::root()."index.php?option=com_guru&controller=guruPrograms&task=buy_action&course_id=".$course_id; ?>">
                                            <?php echo JText::_("GURU_TAKE_COURSE"); ?>
                                        </a>
                                <?php
                                	}
        						...
        						?>
                                        <a class="btn btn-block btn-primary" href="#" onclick="openMyModal('0', '0', '<?php echo JURI::root().'index.php?option=com_guru&view=guruLogin&tmpl=component&returnpage=open_lesson&lesson_id='.intval($lesson_id); ?>'); return false;">
                                            <?php echo JText::_("GURU_TAKE_COURSE"); ?>
...
<?php $programDescription = substr(strip_tags(JHtml::_('content.prepare', $program->description)),0,100);?>

<script>
    (function($){
        $( ".ja-masthead-title" ).html("<?php echo $program->name; ?>");
        if(!$(".ja-masthead-description").length) {
            $( ".ja-masthead-title" ).after('<div class="ja-masthead-description"></div>');
        }
        $( ".ja-masthead-description" ).html("<?php echo str_replace("\r\n","",$programDescription); ?>...");
    })(jQuery);
</script>