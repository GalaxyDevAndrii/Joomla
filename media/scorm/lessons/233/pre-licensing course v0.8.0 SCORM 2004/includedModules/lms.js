...
(function (root, factory) {
    ...
    var lmsReporting = {
        initialize: function () {
            ...
            self.apiWrapper.doLMSSetValue('cmi.completion_status', 'incomplete');
            self.apiWrapper.doLMSSetValue('cmi.success_status', 'unknown');
    ...
    function convertTimeSpanToLmsTimeString(duration) {
    ...
    function saveProgress(progress) {
        ...
        var statusResult = self.apiWrapper.doLMSSetValue('cmi.completion_status', 'incomplete') == 'true';
        var result = progressResult && statusResult;
        if (result) {
            self.apiWrapper.doLMSSetValue('cmi.exit', 'suspend');
            self.apiWrapper.doLMSCommit();
        }
        return result;
    }
    ...
    function removeProgress() {
        ...
        if (result) {
            self.apiWrapper.doLMSSetValue('cmi.exit', '');
            self.apiWrapper.doLMSCommit();
        }
    ...
    function sendCourseResults(course) {
        ...
        self.apiWrapper.doLMSSetValue('cmi.score.raw', getValue(course.result) * 100);
        self.apiWrapper.doLMSSetValue('cmi.score.scaled', getValue(course.result));

        self.apiWrapper.doLMSSetValue('cmi.success_status', getValue(course.isCompleted) ? 'passed' : 'failed');
        self.apiWrapper.doLMSSetValue('cmi.completion_status', 'completed');
